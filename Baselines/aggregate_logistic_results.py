"""Aggregates held-out Logistic Regression results across 5 outer folds."""
...
flags.DEFINE_string(
    "runs_dir",
    None,
    "Directory containing logistic fold outputs. Defaults to ../runs/logistic",
)
flags.DEFINE_string(
    "output_json",
    None,
...
def _resolved_runs_dir() -> Path:
  ...
  return _project_root() / "runs" / "logistic"
...
def _find_fold_summary(runs_dir: Path, fold_num: int):
  ...
  return {
      ...
      "source": summary.get("source", "logistic_regression_final"),
...
def main(argv):
  ...
  if FLAGS.output_json:
    ...
    output_path = _project_root() / "runs" / "logistic_results_summary.json"
