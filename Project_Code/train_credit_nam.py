...
def _update_metrics_and_checkpoints(
  ...
  if FLAGS.debug:
    tf.logging.info("Epoch %d PRAUC Val %.4f", epoch, validation_metric)
...
def _evaluate_best_checkpoint_on_test(
  ...
  for model_index in range(FLAGS.n_models):
    ...
    with tf.Graph().as_default():
      ...
      eval_graph_tensors, test_metric = credit_graph_builder.build_eval_graph(
      ...
      with tf.Session() as sess:
        ...
        test_auc_scores[model_index] = test_metric(sess)
        tf.logging.info(
            "Model %d: Outer Test PRAUC %.4f",
            model_index,
...
def training(x_train, y_train, x_validation, y_validation, x_test, y_test, logdir):
  """Trains the credit-fraud NAM and returns best train/val/test PR AUCs."""
        ...
        for model_index in range(FLAGS.n_models):
          ...
          if epoch % FLAGS.save_checkpoint_every_n_epochs == 0:
            (
                ...
                best_validation_metric[model_index],
                best_train_metric[model_index],
  ...
  for model_index in range(FLAGS.n_models):
    tf.logging.info(
        "Model %d: Best Epoch %d, Train PRAUC %.4f, Validation PRAUC %.4f",
        model_index,
        curr_best_epoch[model_index],
        best_train_metric[model_index],
        best_validation_metric[model_index],
    )
  test_auc_scores = _evaluate_best_checkpoint_on_test(
  ...
  tf.logging.info("Mean Outer Test PRAUC %.4f", np.mean(test_auc_scores))
  return (
      np.mean(best_train_metric),
      np.mean(best_validation_metric),
...
def main(argv):
  ...
  _, _, mean_test_auc = single_split_training(data_gen, test_dataset, FLAGS.logdir)
  tf.logging.info("Completed fold %d with held-out test PRAUC %.4f",
                  FLAGS.fold_num, mean_test_auc)
